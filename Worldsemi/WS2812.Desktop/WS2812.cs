...
// Public surface mirrors GHIElectronics.TinyCLR.Drivers.Worldsemi.WS2812\WS2812.cs.
// The only native call on the device side is NativeFlush; here it's a no-op so
// Desktop consumers can construct the controller and exercise Buffer manipulation
// without hitting MissingMethodException.
namespace GHIElectronics.TinyCLR.Drivers.Worldsemi.WS2812 {
    public class WS2812Controller {
        ...
        // No-op replacement for the InternalCall version in the impl. Discards
        // the LED data — there's no physical pin to bit-bang on Desktop.
        private void NativeFlush(int dataPin, byte[] buffer8, int offset, int size, bool bpp24) { }
    }
}
