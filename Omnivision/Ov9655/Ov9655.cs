...
namespace GHIElectronics.TinyCLR.Drivers.Omnivision.Ov9655 {
    public class Ov9655Controller {
        ...
        public bool Capture() {
            try {
                if (this.data != null) {
                    var size = this.cameraController.Capture(this.data, (int)this.timeout.TotalMilliseconds);

                    FrameReceived?.Invoke(this.data, size);
        ...
        private void SetVga() {
            ...
            Thread.Sleep(2); this.WriteRegister(0x11, 0x01);
            ...
            Thread.Sleep(2); this.WriteRegister(0x09, 0x11);
            Thread.Sleep(2); this.WriteRegister(0x09, 0x01);
