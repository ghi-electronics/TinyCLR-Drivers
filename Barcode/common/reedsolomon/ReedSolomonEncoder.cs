...
namespace GHIElectronics.TinyCLR.Drivers.Barcode.Common.ReedSolomon
{
   ...
   public sealed class ReedSolomonEncoder
   {
      ...
      private GenericGFPoly buildGenerator(int degree)
      {
         ...
         {
            var lastGenerator = (GenericGFPoly)this.cachedGenerators[this.cachedGenerators.Count - 1];
            for (int d = this.cachedGenerators.Count; d <= degree; d++)
            {
               var nextGenerator = lastGenerator.multiply(new GenericGFPoly(this.field, new int[] { 1, this.field.exp(d - 1) }));
